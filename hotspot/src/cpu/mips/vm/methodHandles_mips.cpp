...
//  LP64_ONLY(temp2 = S1);  // used by MacroAssembler::cmpptr
  Label L_ok, L_bad;
...
#define UNPUSH {  __ pop(temp); }
  __ load_klass(temp, obj);
//  __ cmpptr(temp, ExternalAddress((address) klass_addr));
//  __ jcc(Assembler::equal, L_ok);
  __ li(AT, (long)&klass_addr);
  __ ld(AT, AT, 0);
  __ beq(temp, AT, L_ok);
  __ nop();
...
//    __ hlt();           // empty stubs make SG sick
  ...
  Register rdx_argp   = T9;   // argument list ptr, live on error paths
//  Register rax_temp   = rax;
  Register rcx_mh     = S7;   // MH receiver; dies quickly and is recycled
...
//    __ jcc(Assembler::equal, L);
    __ lbu(AT, rbx_method, Method::intrinsic_id_offset_in_bytes());
  ...
  if (ref_kind == 0 || MethodHandles::ref_kind_has_receiver(ref_kind)) {
    __ ld(rdx_argp, Address(rbx_method, Method::const_offset()));
  ...
  if (!is_signature_polymorphic_static(iid)) {
    __ ld(rcx_mh, rdx_first_arg_addr);
    DEBUG_ONLY(rdx_argp = noreg);
  }
...
*/
  ...
  if (iid == vmIntrinsics::_invokeBasic) {
    ...
    case vmIntrinsics::_linkToSpecial:
      ...
      __ ld(rbx_method, member_vmtarget);
      break;
...
//        __ jcc(Assembler::greaterEqual, L_index_ok);
    ...
    case vmIntrinsics::_linkToInterface:
    {
      ...
      __ ld(rbx_index, member_vmindex);
      if (VerifyMethodHandles) {
        Label L;
//        __ cmpl(rbx_index, 0);
//        __ jcc(Assembler::greaterEqual, L);
        __ slt(AT, R0, rbx_index);
        __ bne(AT, R0, L);
        __ nop();
