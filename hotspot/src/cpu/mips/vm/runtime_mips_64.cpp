...
//  OopMapSet *oop_maps = new OopMapSet();
  ...
  /* 2012/9.27 Jin: this frame will be treated as the original caller method.
  ...
  __ sd(FP, SP, fp_off  *wordSize);	// EBP
  ...
  __ move(A0, thread);
  __ li48(T9, (long)OptoRuntime::handle_exception_C);
  __ jalr(T9);
  __ delayed()->nop();
  ...
  /* 2014/5/12 Jin: Fix seg fault when running:
   *    Eclipse + Plugin + Debug As
   *  This is the only condition where C2 calls SharedRuntime::generate_deopt_blob()
   *
   *  Ref:  http://10.2.5.21:8000/projects/java/wiki/Jgj-log-2014-5-12_
   */
  __ move(V0, A0);
  __ move(V1, A1);
